...
parser.add_argument("--repo", default=os.environ.get("GITHUB_REPOSITORY", "your-username/your-repo"), help="GitHub repo for release links")
parser.add_argument("--log", default='build.log', help="Path to the log file")
args = parser.parse_args()

LOG_FILE = args.log
TEMPLATE_FILE = 'README.template.md'
OUTPUT_FILE = 'README.md'
STATE_FILE = 'apps_state.json' # Memory bank
...
# 2. Load previous memory if it exists
...
# 3. Parse the log file line by line
try:
...
for index, app_name in enumerate(sorted(apps_data.keys()), start=1):
    ...
    # Generate the universal download button
    safe_name = app_name.replace(" ", "-").lower()
    download_url = f"https://github.com/{args.repo}/releases?q={safe_name}"
    badge_url = "https://img.shields.io/badge/Download-blue?style=for-the-badge&logo=github"
    download_button = f"[![Download]({badge_url})]({download_url})"

    apps_md += f"<details>\n<summary><b>{index}. {app_name}</b></summary>\n\n"
    
    # Inject the button right at the top of the expanded details view
    apps_md += f"{download_button}\n\n"

    # Add Versions and Bundles
    apps_md += f"* **App Version:** `{data.get('version', 'Unknown')}`\n"
    if repos_list:
...
# Ensure the placeholder matches this script's variable
final_readme = template.replace('{{APPS_LIST}}', apps_md.strip())

# 7. Save the final README
